...
import ci.dgmp.sigefbackend.admin.security.model.dtos.AuthorityDTO;
import ci.dgmp.sigefbackend.admin.security.model.dtos.UserProfileAssoDTO;
import ci.dgmp.sigefbackend.admin.security.model.entities.AuthAssociation;
import ci.dgmp.sigefbackend.admin.security.model.entities.AppAuthority;
import ci.dgmp.sigefbackend.admin.security.model.validators.ExistingUserId;
...
public interface AuthAssoRepo extends JpaRepository<AuthAssociation, Long>
{
    ...
    @Query("select count(vup) > 0 from VUserProfile vup where vup.userId = :userId and vup.profileCode = :profileCode and vup.profileStrId = :strId")
    ...
    // Native query to search privileges with pagination
    @Query(value = """
            select vp.code, vp.name, vp.description, vp.privilege_type_code, vp.privilege_type_name
            from v_privilege vp 
            where (upper(unaccent(vp.name)) like upper(unaccent(:key)) 
                  or upper(unaccent(vp.code)) like upper(unaccent(:key)) 
                  or upper(unaccent(vp.description)) like upper(unaccent(:key)))
            and (:privilegeTypeCodes is null or vp.privilege_type_code in (:privilegeTypeCodes))
            order by vp.name
            LIMIT :limit OFFSET :offset
            """, nativeQuery = true)
    List<Object[]> searchPrivilegesRaw(@Param("key") String key, @Param("privilegeTypeCodes") List<String> privilegeTypeCodes, 
                                     @Param("offset") int offset, @Param("limit") int limit);

    // Count query for searchPrivileges
    @Query(value = """
            select count(*)
            from v_privilege vp 
            where (upper(unaccent(vp.name)) like upper(unaccent(:key)) 
            ...
            and (:privilegeTypeCodes is null or vp.privilege_type_code in (:privilegeTypeCodes))
            """, nativeQuery = true)
    Long countSearchPrivileges(@Param("key") String key, @Param("privilegeTypeCodes") List<String> privilegeTypeCodes);

    // Helper method to search privileges with pagination
    default Page<AuthorityDTO> searchPrivileges(String key, List<String> privilegeTypeCodes, PageRequest pageRequest)
    {
        int page = pageRequest.getPageNumber();
        int size = pageRequest.getPageSize();
        key = "%" + key + "%";

        Long count = countSearchPrivileges(key, privilegeTypeCodes);

        List<Object[]> results = searchPrivilegesRaw(key, privilegeTypeCodes, page * size, size);
    ...
    @Query("""
    select new ci.dgmp.sigefbackend.admin.security.model.dtos.AuthorityDTO(vrp.privilegeCode, vrp.privilegeName, vrp.privilegeDescription, 'PRV', 'PRIVILEGE', vrp.privilegeTypeCode, vrp.privilegeTypeName)
...
""")
    ...
    @Query("""
    select new ci.dgmp.sigefbackend.admin.security.model.dtos.AuthorityDTO(vpp.privilegeCode, vpp.privilegeName, vpp.privilegeDescription, 'PRV', 'PRIVILEGE', vpp.privilegeTypeCode, vpp.privilegeTypeName)
    ...
    List<AuthorityDTO> searchPrivilegesByProfileCode(@Param("profileCode")String profileCode,
                                                     ...
                                                     @Param("privilegeTypeCodes")List<String> privilegeTypeCodes);
    @Query("""
    select new ci.dgmp.sigefbackend.admin.security.model.dtos.AuthorityDTO(vr.code, vr.name, vr.description, 'ROLE', 'ROLE', '', '')
...
""")
    ...
    @Query("""
    select new ci.dgmp.sigefbackend.admin.security.model.dtos.AuthorityDTO(vpr.roleCode, vpr.roleName, vpr.roleDescription, 'ROLE', 'ROLE', '', '')
    ...
    @Query("""
     select new ci.dgmp.sigefbackend.admin.security.model.dtos.AuthorityDTO(vp.code, vp.name, vp.description, 'PRFL', 'PROFILE', '', '')
    ...
    @Query("""
    select new ci.dgmp.sigefbackend.admin.security.model.dtos.AuthorityDTO(vup.profileCode, vup.profileName, vup.profileDescription, 'PRFL', 'PROFILE', '', '')
    ...
    @Query("""
    select new ci.dgmp.sigefbackend.admin.security.model.dtos.AuthorityDTO(vp.code, vp.name, vp.description, 'PRV', 'PRIVILEGE', vp.privilegeTypeCode, vp.privilegeTypeName)
    ...
    @Query("""
    select new ci.dgmp.sigefbackend.admin.security.model.dtos.AuthorityDTO(vrp.roleCode, vrp.roleName, vrp.roleDescription, 'ROLE', 'ROLE', '', '')
    ...
    select vup from VUserProfile vup 
        where vup.userId = coalesce(:userId, vup.userId) 
            ...
            and locate(vup.profileStrChaineSigles, cast(function('getStrChaineSigles',  :strId) as string)) = 1 and 
            (
                locate(upper(coalesce(:key, '') ), upper(cast(function('unaccent',  coalesce(vup.profileCode, '') ) as string))) >0 or
                locate(upper(coalesce(:key, '') ), upper(cast(function('unaccent',  coalesce(vup.profileName, '') ) as string))) >0 or
                ...
                locate(upper(coalesce(:key, '') ), upper(cast(function('unaccent',  coalesce(vup.profileStrName, '') ) as string))) >0 or
                locate(upper(coalesce(:key, '') ), upper(cast(function('unaccent',  coalesce(vup.profileStrSigles, '') ) as string))) >0 or   
                locate(upper(coalesce(:key, '') ), upper(cast(function('unaccent',  coalesce(vup.profileStrSigles, '') ) as string))) >0
            )   
    """)
