package ci.dgmp.sigefbackend.admin.structures.controller.repositories;
...
public interface StrRepo extends JpaRepository<Structure, Long>
{
     ...
     */
    @Query("SELECT new ci.dgmp.sigefbackend.admin.structures.model.dtos.ReadStrDTO(s.strId, s.strName, s.strTypeName, s.strSigle, s.strTypeCode, s.strTel, s.strAddress, s.situationGeo, s.parentId, s.parentName, s.parentSigle, s.strLevel, s.chaineSigles) FROM VStructure s WHERE s.parentId IS NULL")
     ...
     */
    @Query("SELECT new ci.dgmp.sigefbackend.admin.structures.model.dtos.CreateOrUpdateStrDTO(s.strId, s.strName, s.strSigle, s.strType.code, s.strParent.strId, s.strTel, s.strAddress, s.situationGeo) FROM Structure s WHERE s.strId = :strId")
     ...
     */
    @Query("SELECT new ci.dgmp.sigefbackend.admin.structures.model.dtos.ChangeAnchorDTO(s.strId, s.strType.code, s.strParent.strId, s.strName, s.strSigle, s.strTel, s.strAddress, s.situationGeo) FROM Structure s WHERE s.strId = :strId")
